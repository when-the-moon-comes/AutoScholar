...
DEFAULT_FIELDS = "paperId,title,year,authors,venue,url,abstract,citationCount,externalIds"
...
def load_candidates(path: Path) -> list[dict]:
    ...
    if isinstance(payload, dict) and isinstance(payload.get("candidates"), list):
        ...
        candidates = []
        for track, key in (("A", "track_a_candidates"), ("B", "track_b_candidates")):
            for candidate in payload.get(key, []):
                if isinstance(candidate, dict):
                    normalized = dict(candidate)
                    normalized.setdefault("track", track)
                    candidates.append(normalized)
    else:
        candidates = payload
    if not isinstance(candidates, list):
        raise ValueError(
            "innovation_candidates payload must contain 'candidates' or "
            "'track_a_candidates'/'track_b_candidates' lists"
        )
    validated = []
...
def retry_sleep_seconds(exc: httpx.HTTPError, sleep_429: float, attempt: int) -> float:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if retryable and attempt < max_retries:
                time.sleep(retry_sleep_seconds(exc, sleep_429, attempt))
                continue
            break
...
def main() -> int:
    ...
    client = SemanticScholarClient(timeout=args.timeout)
    try:
        for candidate in candidates:
